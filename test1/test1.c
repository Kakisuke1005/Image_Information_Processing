...
{
  unsigned char A[3][IMAGE_SIZE][IMAGE_SIZE];
  ...
  for(int i=width/2;i<width;i++){
    for(int j=height/2;j<height;j++){
      for(int k=0;k<3;k++){
	imgout[k][i][j]=A[k][width-i-1][height-j-1];
      }
    }
  }
